...
class UserModel {
  constructor(client) {
    this.db = client.db("HackConcordia"); // Ensure "HackConcordia" is the correct database name
    this.collection = this.db.collection("userData"); // Ensure "userData" is the correct collection name
  }

  async getAllUsers() {
    try {
      const users = await this.collection.find().toArray(); // Use find() to get all users
      return users;
    } catch (error) {
  ...
  async updateUserPet(email, petType, petName) {
    const result = await this.collection.updateOne(
      { email },
      { 
        $set: { 
